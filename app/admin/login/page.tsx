...
export default async function AdminLoginPage({ searchParams }: { searchParams?: Promise<{ error?: string }> }) {
  ...
  return (
    <section className="mx-auto grid min-h-[80vh] max-w-5xl place-items-center px-4 py-12 sm:px-6 lg:px-8">
      <div className="grid w-full gap-6 lg:grid-cols-[0.95fr_1.05fr] lg:items-stretch">
        <Card className="space-y-6 bg-[linear-gradient(135deg,rgba(15,23,42,0.96),rgba(30,41,59,0.92))] text-white shadow-soft dark:bg-[linear-gradient(135deg,rgba(241,245,249,0.98),rgba(226,232,240,0.92))] dark:text-slate-950">
          <div className="space-y-3">
            ...
            <p className="text-sm leading-6 text-white/75 dark:text-slate-700">
              Use the allowlisted admin email and password. This page stays separate from the public login flow.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            <Badge className="border-white/15 bg-white/10 text-white/80 dark:border-slate-200 dark:bg-white/70 dark:text-slate-700"><ShieldCheck className="mr-1 h-3.5 w-3.5" /> Secure CRM</Badge>
          </div>
        </Card>
