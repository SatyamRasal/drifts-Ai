  ...
  return (
    <section className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <Card className="mt-8 space-y-4">
        ...
        <p className="text-sm text-slate-600 dark:text-slate-300">Requests are attached to your signed-in identity so the support queue can be handled without manual reconciliation.</p>
      ...
      {submitted ? <div role="status" aria-live="polite" className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-700 dark:border-emerald-900 dark:bg-emerald-950 dark:text-emerald-300">Your support request was submitted.</div> : null}

      {!isLoggedIn || !session ? (
        <Card className="mt-8 space-y-4">
          <div className="text-lg font-semibold">Login required</div>
          <p className="text-sm text-slate-600 dark:text-slate-300">Support submissions must come from a verified session.</p>
          <Link href="/login?next=/support" className="inline-flex items-center justify-center rounded-2xl bg-slate-950 px-5 py-3 text-sm font-medium text-white hover:bg-slate-800 dark:bg-white dark:text-slate-950 dark:hover:bg-slate-200">Sign in to continue</Link>
        </Card>
      ) : (
        <Card className="mt-8 space-y-4">
          ...
          <form action={createLead} className="grid gap-4 md:grid-cols-2">
            <input type="hidden" name="leadType" value="support" />
            <input type="hidden" name="website" value="" />
            <input type="hidden" name="redirectTo" value="/support?submitted=1" />
            ...
            <div className="md:col-span-2"><label className="mb-1 block text-sm">Message</label><Textarea name="message" rows={8} placeholder="Describe the issue or the feedback you want to share." minLength={10} required /></div>
            <div className="md:col-span-2"><Button>Send support request</Button></div>
          </form>
        </Card>
      )}
    </section>
